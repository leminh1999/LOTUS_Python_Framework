...
class indentMan():
  ...
  def genGapTree(self):
    gapTreeLevel = self.level
    if self.treeEna == 1:
      gapTree = ""
      while gapTreeLevel >= 1:
        gapTree += "| "
        gapTreeLevel -= 1
      gapTree += "\n"
    else:
      gapTree = ""
    self.gapTree = gapTree
    self.gapTreeFlag = 1
  
  def indentStringGen(self):
    if self.treeEna == 1:
      self.tree = ""
      treeLevel = self.level
      while treeLevel > 1:
        self.tree += "| "
        treeLevel -= 1
      self.tree += "|_"
  ...
  def gen(self):
    if self.gapTreeFlag == 1:
      self.gapTreeFlag = 0
      # print("DEBUG: "+self.gapTree)
    else:
      self.gapTree = ""
    return self.gapTree+self.tree
...
NEW_FORMAT2 = '%(message)-75s [%(asctime)s] [%(levelname)5s] %(filename)s:%(funcName)s()[Line %(lineno)d]'
