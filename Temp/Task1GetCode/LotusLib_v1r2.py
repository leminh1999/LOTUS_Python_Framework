...
class LotusLib ():
  ...
  #########################################################
  # Name: openProgram
  # Function: Mở một chương trình và tiếp tục đi tiếp các dòng lệnh
  #           code phí bên dưới (song song).
  # Parameter:
  #   + exePath: Đường dẫn đến chương trình exe thực thi.
  # Return: None.
  # Ex: taskRunParallel("C:\Program Files\CyberGhost 8\Dashboard.exe")
  #########################################################
  def openProgram (exePath = ""):
    from subprocess import check_output as winRun
    winRun(exePath, shell=True).decode()